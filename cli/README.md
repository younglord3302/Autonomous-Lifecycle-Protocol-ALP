...
| `alp vault` | Encrypted secrets store (v8.4.0): `set` / `get` / `list` / `rotate` / `audit` |
...
alp vault get db-password --key maintainer.key   # (v8.4.0)
```
...
✅ **Stable** — Production-Grade Era (V5), toolchain `8.4.0`. The `@alp/cli`
